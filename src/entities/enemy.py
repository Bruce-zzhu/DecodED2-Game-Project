...
class Enemy(Entity):
    ...
    def __init__(self, coords, speed, image):
        ...
        self.turn = False
        self.direction = 1  # initially go to right
        self.speed = speed
        self.id = self.static_id
        self.static_id += 1

    def tick(self, delta, objects):
        # turn enemy object around upon hitting a wall
    ...
    def random_enemy_shoot(objects, num_enemies, delta, NUM_ENEMIES_SHOOT = 1):
        ...
        if Enemy.Bullet_cooldown >= ENEMY_BULLET_COOLDOWN:
            ...
            for e in enemies_to_shoot:
                objects.append(Bullet(Vector2(e.x, e.y), ENEMY_BULLET_SPEED, KILL_PLAYER=True))
